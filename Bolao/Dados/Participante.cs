...
namespace Bolao.Dados {
	public class Participante {
		...
		public static List<Participante> BuscarParticipantes(int? codigoUsuario, int? codigoBolao, string statusBolao) {
			string sql =
				"declare " +
					"@codUsuario int = ?, " +
					"@codBolao int = ? " +
				...
				"where (@codUsuario = -1 or p.cod_usuario = @codUsuario) " +
					...
					(!String.IsNullOrEmpty(statusBolao) ? "and b.ind_status = '" + statusBolao + "' " : "") +
